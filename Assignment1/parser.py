...
class Parser:
    ...
    def find_error(self):
        ...
        for i, token in enumerate(self.tokenlist):
            if(token.kind == TokenKind.ID and
               i-1 >= 0 and self.tokenlist[i-1].kind == TokenKind.ID):
    ...
    def is_connective(self, kind):
        if(kind == TokenKind.AND or
           kind == TokenKind.OR or
           kind == TokenKind.IMPLIES or
           kind == TokenKind.IFF):
            return True
        else:
            return False
    ...
    def is_compound(self):
        if(len(self.tokenlist) > 1 and
           self.is_connective(self.tokenlist[1].kind) or
           self.top_kind() == TokenKind.LPAR or
           self.top_kind() == TokenKind.NOT):
            return True
        else:
            return False
    ...
    def error(self, s, token):
        if token is not None and s is None:  # general location
            self.errors.append(
                '(line' +
                str(token.loc.line) +
                ',col' +
                str(token.loc.col) +
                ')')
        elif s is not None and token is None:  # general type of error
            self.errors.append(s)
        elif token is not None and s is not None:  # type and location of error
            self.errors.append(
                s +
                ' (line' + str(token.loc.line) +
                ',col' +
                str(token.loc.col) +
                ')')
    ...
    def str_kind(self, kind):
        if kind is TokenKind.ID:
            ...
            return 'NOT'
        elif(kind is TokenKind.AND or
             kind is TokenKind.OR or
             kind is TokenKind.IMPLIES or
             kind is TokenKind.IFF):
            return 'CONNECTIVE'
